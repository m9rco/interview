...
面向后台技术面试的通用复习站点，覆盖互联网/智能硬件后台、游戏基础架构与游戏业务。每个专题按五段式组织：**场景问题 → 实现方案 → 为什么这么做 → 为什么别的选择不行 → 沉淀结论**。

- 在线（GitHub Pages）：https://m9rco.github.io/interview/
- 在线（云对象存储 COS）：绑定自定义域名（后续扩充启用）
...
npm run docs:build:cos    # COS 目标（base=/，绑定自定义域名）
```
