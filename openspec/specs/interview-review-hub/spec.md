# interview-review-hub

## Purpose

面试复习中心 `guide/index.html` 的能力规范：定义单页动态 HTML 首页所需具备的可视化拓扑、主题切换、独立分册、离线运行、可扩展数据结构、响应式与打印友好等交互与非功能约束。规范以"任何专题追加只需向 `THEMES` 数组追加一项"为核心可扩展目标，随着专题目录扩展（当前 13 个）而共同演进。

## Requirements

### Requirement: 首页拓扑可视化

复习中心 `guide/index.html` MUST 提供中央 SVG 拓扑（或在小屏折叠为竖向列表）呈现全部专题节点、节点间关系连线（区分技术依赖与面试联问两类），且每个节点 MUST 可点击、可键盘 Tab 聚焦、可通过 Enter/Space 触发主题切换。

#### Scenario: 首次打开时展示全部节点

- **WHEN** 用户双击 `guide/index.html`
- **THEN** 页面在无网络环境下 500ms 内渲染出中央 SVG 拓扑（或小屏下的竖向主题列表），节点数量与 `THEMES` 数组长度一致，每个节点显示图标 + 中文标题
...
`guide/index.html` MUST 提供顶部标签栏（tab strip）或侧边栏作为拓扑之外的第二切换入口，且 MUST 保证任何一个专题被激活时，详情面板严格按"是什么 → 为什么这么选 → 踩过什么坑 → 怎么填的"四段式渲染其内容。

#### Scenario: 通过标签栏切换主题

- **WHEN** 用户点击顶部标签栏或侧边栏中的任一入口
- **THEN** 详情面板切换到对应主题内容，URL hash 更新，被选中项视觉上区别于其他项
...
`THEMES` 数组中的每一个专题 MUST 存在一份**独立可离线打开的 HTML 分册文件**（`guide/theme-<slug>.html`），且这些分册 MUST 从统一的 HTML 骨架生成或由首页派生（切 `__GUIDE_MODE__ = 'single'` + 注入 `__GUIDE_THEME_ID__`），仅注入该专题自身的内容数据。
...
主题内容 MUST 以内联 JavaScript 中的 `THEMES` 数组表达（源在 `guide/_themes.js`，同时内联到 `guide/index.html` 与各分册），且新增一个专题 MUST 只需向该数组追加一项配置（可选地生成一份独立分册），无需修改渲染器或样式表核心逻辑。

#### Scenario: 追加一个专题

- **WHEN** 开发者向 `THEMES` 数组新追加一个 `{ id, title, icon, tagline, hero?, stages, sources }` 对象
- **THEN** 首页顶部标签栏/侧边栏自动出现该专题入口、拓扑节点计数与详情渲染工作无需改核心代码即完成

#### Scenario: 章节类型可组合

- **WHEN** 一个专题的 `stages` 中混合使用 `md | kv | quote | table | code | ascii | callout` 等章节类型
...
- **WHEN** 视口宽度 < 768px
- **THEN** 中央 SVG 拓扑折叠为竖向的主题列表（icon + 标题），点击行为与桌面一致；标签栏改为可横向滑动或折叠为汉堡菜单
