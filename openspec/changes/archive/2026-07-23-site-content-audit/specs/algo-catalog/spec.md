...
算法域（`/algo/`）内容目录规范：为该域每个专题定义"关键知识点必含项"最低门槛。此前该域无任何规范覆盖。既有 8 专题（`data-structures`、`sorting`、`binary-search`、`two-pointers-sliding-window`、`dynamic-programming`、`graph`、`backtracking`、`backend-algorithms`）持续有效，本 delta 重点约束 7 个待补的高频编码考点专题。
