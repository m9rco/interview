...
> **打个比方**：房间推荐像**共享单车 App 的附近调度**——地图上先按**距离近、电量足、锁没坏**给每辆车打融合分（对应 ZSet 里"热度 + 新鲜度"），谁分高谁排前面；同时**空闲太久 / 掉线的车辆自动从推荐里下架**（TTL + 定时扫清脏 member），别再骗用户跑一趟发现车不见了；用户看到列表后还会按自己的**段位/习惯精排**（应用层按玩家匹配度做二次微调）。**类比失效边界**：共享单车调度是**分钟级**——中央系统慢几分钟没啥事；游戏房间是**秒级刚需**——一间热门房被同时推荐给几十个玩家，如果不加**预扣位**（推荐即软占一个槽、进房失败再释放），就会出现"**列表看着有位，一点进去满员**"的进房失败率飙升，比"看到车没了"糟糕十倍。所以缓存的强一致性、热点房间的并发保护，是共享单车那套调度不用面对、房间推荐必须补上的一层。
...
1. 房间推荐列表为什么用 ZSet 而不用 MySQL `ORDER BY score DESC LIMIT`？请对比两者在读、写、实时性上的差异。

<details><summary>参考答案</summary>

ZSet 更新 member 分值是 **O(log N) 内存跳表**操作、读用 `ZREVRANGE` **免现算排序**、内存态**秒级增删**；MySQL 排序列高频更新**写放大**、每次查询**现算排序**、**深翻页恶化**、索引维护贵，撑不住大厅量级。

</details>

2. 融合分值由哪几个信号组成？为什么匹配度不能像热度那样直接写进全局 ZSet？

<details><summary>参考答案</summary>

融合分 = **热度 × 时间衰减 × 匹配度**。热度、新鲜度是房间自身属性可入 ZSet；**匹配度依赖"谁在看"**（玩家段位/模式），无法预先固化，故基础分入 ZSet **粗排**、匹配度在应用层**精排**。

</details>

3. 为什么必须做分值时间衰减？不做会怎样？

<details><summary>参考答案</summary>

不衰减则早创建、一度很热的**老房间永远霸榜**，即便已冷清或将解散。时间衰减让分值**随时间自然回落**，把展示机会让给新鲜活跃的房间，与资讯流时间衰减同理。

</details>

4. offset 分页（`ZREVRANGE start stop`）在数据实时变动时有什么问题？如何解决？

<details><summary>参考答案</summary>

数据重排时按 rank 偏移会**漂移**，翻页出现**重复或漏项**。解决：改用 **score 游标**，以"上一页最后一个分值"为界 `ZREVRANGEBYSCORE (lastScore -inf LIMIT 0 size`，避免偏移漂移。

</details>

5. 为什么要按模式/段位分桶？脏 member（ZSet 有 ID 但详情已过期）怎么清理？

<details><summary>参考答案</summary>

单个全局 ZSet 会成**大 key**（阻塞、迁移难、热点），分桶打散且**天然贴合查询习惯**、隐含匹配度。脏 member 三管齐下：详情 Hash 设 **TTL** 兜底、**定时扫描** `ZREM`、**读路径**遇空详情顺手 `ZREM` 自愈。
