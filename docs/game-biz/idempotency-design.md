...
> **打个比方**：幂等键就像**快递取件码**——同一个包裹只对应**同一串码**，你到柜前**扫一次码，柜门开、包裹取走、码就作废**；接下来无论是快递员再扫、你自己再来一次、还是别人拿相同的码来碰运气，柜子都只会告诉你"**该包裹已被取走**"，绝不会凭空多吐出一份。**类比失效边界**：取件码的作废是**服务端的柜机强校验**，玩家/快递员本地记不记都不影响；如果幂等键只写在**客户端本地缓存**里做"重复点击拦截"，一旦用户**换设备、清缓存、卸载重装**，客户端就以为是首次请求，本地拦截彻底失效——所以幂等键**必须由发起方生成、但由服务端持久化并校验**，不能把最后一道关设在能被玩家清空的地方。
...
::: tip 与商城支付实战的呼应
本域《业务代理 · 支付 · 商城》里支付代理的"幂等四道闸"（未找到订单 / 已终止 / **重复发货返回成功** / 正常）正是这套范式的落地：**重复发货返回成功**对应本文"重复请求幂等返回成功、避免上游无限重试"；商城服务发货前查 `received_list`、订单优先读 cache miss 再回查 DB，对应"去重表 + 先查快路径"。
:::
...
综合整理自分布式幂等性设计的通用实践与游戏支付/发奖链路的落地经验；具体案例呼应本域《业务代理 · 支付 · 商城》中支付代理幂等四道闸与商城服务重复发货处理的实战总结。

## 自测：合上资料能说清楚吗？

1. 幂等键为什么必须由发起方生成、并在重试中保持不变？如果服务端每次现生成会怎样？

<details><summary>参考答案</summary>

幂等的前提是能识别**"同一个请求"**。若服务端每次收到才生成键，重试就成了**新请求**，键不同，幂等失效、重复处理照样发生。所以客户端重试必须带**同一个** `request_id`/`txn_id`。

</details>

2. "先查后写"（if 没处理过 { 处理 }）在并发下为什么会资损？

<details><summary>参考答案</summary>

查和写**不是原子**的，中间有并发窗口。两个请求可能**同时通过**"没处理过"判断，各处理一次（各发一份奖）。必须靠唯一索引/CAS 把查改压成一步，消除窗口。

</details>

3. 对比 Redis SETNX 去重令牌 与 DB 唯一索引：各自的定位是什么？为什么要叠加？

<details><summary>参考答案</summary>

**SETNX 是性能快门**：高并发下先拦大部分重复、减轻 DB 压力，但会因超时/宕机/TTL 到期而**漏放**，不保证正确性。**唯一索引是正确性底线**：由 DB 存储层原子保证，兜住漏网并发，但每个重复都打到 DB。故叠加：快门挡量、索引兜底。

</details>

4. 为什么"至少一次投递 + 处理幂等"约等于"恰好一次"？为什么不直接做精确一次投递？

<details><summary>参考答案</summary>

网络层无法区分**"对方没收到"和"收到但 ack 丢了"**，精确一次投递几乎做不到。工程解法：投递层做**至少一次**（重试到成功），处理层做**幂等**（重复无副作用），两者叠加达到**效果恰好一次**（effectively-once）。

</details>

5. 状态机 + CAS 适用什么场景？纯"新增型"操作（如发一次性奖励）为什么不能只靠它？

<details><summary>参考答案</summary>

CAS 适用**有生命周期的实体**（如订单），靠"当前是旧状态才能改"实现原子流转、天然幂等。但纯新增操作**没有可流转的旧状态**，无从 CAS，需靠**唯一索引**去重兜底。
