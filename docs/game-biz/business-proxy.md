...
> **打个比方**：业务代理就像**大厦前台**——玩家进来先**报姓名（会话保持/鉴权）**，前台**核对预约单（票据校验）**并**贴访客证（会话 token）**，然后**领你去对的部门**（按命令字/uid 路由到大厅或商城），进门前还顺便做**安检脱敏**（把身份证号打码、把敏感字段过滤后再往里递）。同一个访客证在整栋楼里全程有效，来回穿梭不用重开门禁。**类比失效边界**：真前台只做"**人物匹配**"这一件事，业务代理还得处理**协议翻译**（外部 HTTP/JSON ⇄ 内部 TCP/protobuf、甚至 KCP）和**背压回压**（下游发货服务慢了要主动挡住外面的洪水，而不是让访客在大厅无限堆积）——这两件事前台完全没有类比可套，也是把"代理"当成"HTTP 转发器"最容易翻车的两个盲区。
