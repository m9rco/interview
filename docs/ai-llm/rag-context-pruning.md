...
> 本篇是 [RAG 主线](./rag.md) **检索后 / 上下文构造**环节的实现级深潜，主线只做概览、剪枝细节在此展开。
...
<svg viewBox="0 0 660 260" width="100%" style="max-width:660px;height:auto" role="img" aria-label="Listwise 剪枝：8 条 chunk 交给 grader，保留 3 条有用、丢弃 5 条冗余">
  <text x="20" y="16" font-size="11" fill="currentColor">检索候选（top_m=8）</text>
  <g font-size="11" fill="#e2e8f0">
    <rect x="20" y="26"  width="120" height="22" rx="4" fill="#334155"/><text x="30" y="41">chunk 1</text>
    <rect x="20" y="52"  width="120" height="22" rx="4" fill="#334155"/><text x="30" y="67">chunk 2</text>
    <rect x="20" y="78"  width="120" height="22" rx="4" fill="#334155"/><text x="30" y="93">chunk 3</text>
    <rect x="20" y="104" width="120" height="22" rx="4" fill="#334155"/><text x="30" y="119">chunk 4</text>
    <rect x="20" y="130" width="120" height="22" rx="4" fill="#334155"/><text x="30" y="145">chunk 5</text>
    <rect x="20" y="156" width="120" height="22" rx="4" fill="#334155"/><text x="30" y="171">chunk 6</text>
    <rect x="20" y="182" width="120" height="22" rx="4" fill="#334155"/><text x="30" y="197">chunk 7</text>
    <rect x="20" y="208" width="120" height="22" rx="4" fill="#334155"/><text x="30" y="223">chunk 8</text>
  </g>

  <!-- grader -->
  <rect x="250" y="96" width="150" height="72" rx="10" fill="#7c3aed"><animate attributeName="opacity" values="0.55;1;0.55" dur="3s" repeatCount="indefinite"/></rect>
  <text x="325" y="126" text-anchor="middle" font-size="13" fill="#fff">listwise grader</text>
  <text x="325" y="146" text-anchor="middle" font-size="10" fill="#e9d5ff">通读全部候选 · 5 档打分</text>
  <g stroke="#64748b" stroke-width="1" opacity="0.5">
    <line x1="140" y1="37"  x2="250" y2="120"/><line x1="140" y1="63"  x2="250" y2="124"/>
    <line x1="140" y1="89"  x2="250" y2="128"/><line x1="140" y1="115" x2="250" y2="132"/>
    <line x1="140" y1="141" x2="250" y2="136"/><line x1="140" y1="167" x2="250" y2="140"/>
    <line x1="140" y1="193" x2="250" y2="144"/><line x1="140" y1="219" x2="250" y2="148"/>
  </g>

  <!-- kept (green) -->
  <g font-size="11" fill="#fff">
    <rect x="500" y="60"  width="140" height="24" rx="4" fill="#16a34a"><animate attributeName="opacity" values="0;0;1" dur="4s" repeatCount="indefinite"/></rect><text x="512" y="76">✓ chunk 3（保留）</text>
    <rect x="500" y="92"  width="140" height="24" rx="4" fill="#16a34a"><animate attributeName="opacity" values="0;0;1" dur="4s" repeatCount="indefinite"/></rect><text x="512" y="108">✓ chunk 5（保留）</text>
    <rect x="500" y="124" width="140" height="24" rx="4" fill="#16a34a"><animate attributeName="opacity" values="0;0;1" dur="4s" repeatCount="indefinite"/></rect><text x="512" y="140">✓ chunk 6（保留）</text>
  </g>
  <!-- dropped (fade + fall) -->
  <g font-size="11" fill="#94a3b8">
    <rect x="500" y="168" width="140" height="20" rx="4" fill="#334155"><animate attributeName="opacity" values="0.6;0.6;0.15" dur="4s" repeatCount="indefinite"/><animateTransform attributeName="transform" type="translate" values="0 0;0 0;0 22" dur="4s" repeatCount="indefinite"/></rect><text x="512" y="182">✗ 冗余 ×5 丢弃</text>
  </g>
  <text x="470" y="238" font-size="11" fill="currentColor">902 → 196 token（挤掉 78%），召回不降反升</text>
</svg>
...
> **打个比方**：上下文剪枝就像**做菜前的配菜**——一大堆食材从冷柜拿出来（rerank top_m 候选），按菜谱只挑真正下锅的那几味（listwise grader 通读打分 1~5），剩下的整整齐齐放回去；固定 top-N 相当于闭着眼数三个食材，简单菜够用、多味杂菜不是漏关键就是塞太满。**类比失效边界**：现实里锅越大装越多，模型的上下文窗口却是**硬约束**且有"迷失在中间"——长上下文中段最容易被模型忽略，塞得越满召回反而越低；所以剪枝不只是省 token，更要**把真正的关键证据放到 prompt 头尾**、把冗余从中段挤走，且 pruner 是 reranker 的放大器——**上游没召回的关键 chunk，剪枝层再聪明也变不出来**（呼应"下游只能删不能补"）。
...
**方法**
- **Q：listwise 和 pointwise 的本质区别？** A：listwise 让模型**一次看完所有候选再打分**，能利用 chunk 间的冗余/互补关系；pointwise 各打各的、互不可比。
...
本地复现实验（`rag_pruner/` + OpenSpec change `add-listwise-context-pruner`），原方法出自 [kapa.ai — How we prune RAG context](https://www.kapa.ai/blog/how-we-prune-rag-context)（2026-07 复现；数字为本机 CPU + 小型 grader 实测，换模型/换 reranker 会漂，方向性结论以分层归因为准）。

## 自测：合上资料能说清楚吗？

1. 检索已经做了 rerank，为什么还要再插一层剪枝？
<details><summary>参考答案</summary>

rerank 是 **pointwise**：对每条 chunk 单独打分，分不出「两条**重复**只留一条」与「两条**各含答案一半**都得留」；且过长上下文**稀释注意力**（迷失在中间）、多花 token。

</details>

2. 对比 pointwise rerank 与 listwise grader 的本质差异，为什么后者能处理多跳问题？
<details><summary>参考答案</summary>

pointwise 各打各的、**分数互不可比**；listwise **一次通读全部候选再打分**，能利用 chunk 间的**冗余/互补**关系，为每题算出合适的 N，从而在多跳题保住互补 chunk。

</details>

3. threshold 和 keep_top_k 各管什么？为什么要两个旋钮？
<details><summary>参考答案</summary>

**threshold**（默认 3）是主旋钮，调**压缩率↔recall** 取舍；**keep_top_k**（默认 2）是**兜底**，永远保留 rerank 前 K 条，防 grader **抽风**误删。

</details>

4. 实验里「pytest 全绿」和「本机净 token 为负」为什么都不能否定方法？
<details><summary>参考答案</summary>

三层假设别混：**H1 代码正确**（pytest 保证，与模型无关）；**H2 方法有效**（压缩↑且 recall↑，已强通过）；**H3 成本正负**是**模型选型**问题——用 23GB 大模型当 grader 才为负，换小模型即翻正。

</details>

5. 什么场景**不该**上 listwise 剪枝？
<details><summary>参考答案</summary>

**秒级同步 UI + 已很锐利的 reranker**（多 0.7s 用户有感）；**检索集本来就小**（top-3/4，pruner 只能删不能补）；**还没有标注评测集**（旋钮无锚点）。核心：pruner 是 reranker 的**放大器不是替代品**。
