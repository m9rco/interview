...
```
┌──────────────────────────────────────────────────┐
│  阶段 1：双写（新旧集合同时写入，查询走旧集合）        │
│  阶段 2：后台异步重建新集合（重新 embed 所有 chunk）   │
│  阶段 3：流量切换（查询切到新集合）                   │
│  阶段 4：确认稳定后删除旧集合                        │
└──────────────────────────────────────────────────┘
```
...
```
1. 审计
   ├── 统计各 source 的 chunk 数 & 最旧时间
   ├── 识别孤儿 chunk（源文件已删）
   └── 识别过期 chunk（超 TTL）

2. 删除
   ├── 孤儿 chunk → 直接删
   ├── 过期 chunk → 标记 deprecated 或直接删
   └── 语义重复簇 → 保留最新，删其余

3. 修复
   ├── 元数据缺失 → patch 补全
   └── 分块策略变更 → 重新分块 + 重 embed（指定 source）

4. 模型升级
   └── 蓝绿双写 → 后台重建 → 流量切换 → 删旧集合

5. 验证
   ├── 抽样 query 对比清理前后 Recall@10
   └── 监控向量库 chunk 总数、平均质量分
```
