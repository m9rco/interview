...
> **打个比方**：Prefill 和 Decode 就像**外卖厨房**——Prefill 是"一次性把上下文炒好"（并行处理长 prompt，算力烧得凶但只烧一次），Decode 是"一口一口出饭"（逐 token 自回归、看似轻但要烧到吃完）。**前缀缓存**就是**预制半成品**：把稳定的 system prompt/few-shot 前段"半成品"存下来，下一单直接热一下，省的是 Prefill 那一大炒。**类比失效边界**：预制菜复用的前提是**前缀完全一致**——只要中间插一个字（比如把时间戳、用户名塞在 system prompt 里），缓存立刻失效、要从头再炒；所以工程上要把"稳定的系统提示 + few-shot"钉死在最前面、把"变量部分"放在最后，才能真正吃到 prefix caching 的折扣，否则命中率会低得让账单毫无变化。
