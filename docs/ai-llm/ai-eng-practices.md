...
- **Q：企业级工程规范全景有哪几段？** A：开发期（lint/format/静态分析/分支策略）→ 提交评审期（Conventional Commits/CODEOWNERS/pre-commit）→ CI 门禁（测试+覆盖率/SAST/SCA/可复现构建）→ 发布运维期（SemVer/灰度回滚/可观测审计），事故复盘 ADR 回流开发期形成闭环。
