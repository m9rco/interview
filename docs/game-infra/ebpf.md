...
> **打个比方**：eBPF 像给内核开了个"**受监管的实习生工位**"。以前想让内核干点定制的活，要么改内核源码重新编译（大动干戈还得重启），要么写内核模块——那等于让个没证的人直接进手术室主刀，一崩就是整机宕机。eBPF 则是在内核里设了个实习生岗：你写的小程序上岗前先过一道极严的**入职审查（verifier）**——不许有无界循环、不许越界访存、必须能证明一定会正常退出；审查通过才准干活，还给配上内核态的原生性能（**JIT** 编译成机器码），产出通过 **Map**（一个内核态/用户态共享的"公文柜"）交换出来。**类比失效边界**：这实习生被管得死死的——verifier 为了担保"绝不把内核搞崩"，禁掉了无界循环、卡死了栈大小和指令条数、能调的内核函数也是白名单。所以 eBPF 只擅长在钩子点上做"快速判断 / 统计 / 改包"这类有界小活，干不了任意复杂的通用计算；真正复杂的逻辑，还得老实回到用户态去写。
