...
> 即：**用寻址规则决定连接方向 + 每节点只维护常数个 peer**，把 O(N²) 压到接近 O(N) 甚至线性可控。相关做法见 [NZMesh × K8s](/game-infra/nzmesh-k8s.md) 与 [蓄水池抽样选节点](/game-infra/reservoir-sampling.md)。
...
**相关专题**：[Raft 与 Gossip 协议](/game-infra/raft-gossip.md) · [Istio 与 Cilium 服务网格](/game-infra/mesh-istio-cilium.md) · [NZMesh × K8s 部署](/game-infra/nzmesh-k8s.md) · [蓄水池抽样](/game-infra/reservoir-sampling.md)
