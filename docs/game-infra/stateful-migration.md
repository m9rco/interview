...
sequenceDiagram
    ...
    participant R as 路由(消息总线/一致性哈希表)
...
> **打个比方**:在线迁移就像**居民搬家 + 双钥匙交接**。旧家先**冻结门锁**(freeze 写),不让新客人进;把家当**打包发去新家**(快照 + 增量);新家开门收拾好后,**换发新地址**给邮政和朋友(切路由)。中间"锁旧门 + 通知邮政"的空档窗口,是不可避免的"暂不接客"。**类比失效边界**:搬家可以让房子空半天,玩家却不接受"这几秒暂时不能打技能"。所以工程上要把**冻结窗口压到毫秒级**(靠增量先追到几乎为零再冻),并且让**新连接优先落到新节点**(客户端重连即命中新家),而不是死等旧节点把最后一批数据推完再放行——用"提前重路由 + 幂等回放"替代"等搬完再开门",才是玩家无感的真正工程含义。
...
无状态请求**自包含、状态在 DB**,漂移无损。有状态服务的**内存态**(战斗上下文、AOI、房间实时状态)不落 DB 或落得很粗,**连接态**是长连接需重路由,且半局状态**不可重算**。直接重建=数据丢失,必须走在线迁移。
...
快照期间源仍在处理写,快照到达目标端时已**过时(stale)**,不追平会丢掉快照后的变更,故**增量不可省**。切流与最后追平之间若有新写落源,就会丢;**冻结写**保证切换瞬间源与目标严格一致,是零丢失的必要条件。冻结只在增量已很小时的一瞬,玩家近似无感。
...
网络**重传**、协调器**重试**都可能让同一批 op 送达两次。做法:每个 op 带全局单调 **seq**,目标端记 **lastApplied** 去重,seq ≤ lastApplied 直接跳过;并尽量把操作设计成**幂等语义**(set 而非 incr),使重复应用无副作用。
...
**迁移**面向计划内变更:**源健在**,可慢慢增量追平、留回滚点、失败退回,追求玩家无感。**恢复**面向故障:**源已宕/失联**,没有实时增量流,只能靠落盘的 **checkpoint + WAL 回放/副本接管**重建到最近可用点,追求 **RPO/RTO**。一句话:迁移有活源,恢复只有落盘历史。
...
**逐 slot 滚动**:锁定待迁 slot → 该 slot 内房间/玩家走快照+增量流程 → 切路由表 → 解锁,任一时刻只有一小撮 slot 短暂冻结。好处是把**爆炸半径**关进单个 slot,失败只影响局部,且可暂停/续传。
