...
::: tip 一句话结论
游戏定制分布式 NoSQL：缓存落地融合、分表因子 hash 分片、无状态接入+主备存储，扛百万 QPS。
:::

## 场景问题

> **打个比方（缓存落地融合）**：这套自研游戏 KV 把"缓存"和"落地存储"揉成了一体，像**前店后厂**——玩家读写打到内存这个"门店"，秒回（<10ms）；系统在后台异步把数据搬进磁盘"仓库"落袋为安，玩家全程感觉不到仓库的存在。这跟常见的"Redis 当缓存 + MySQL 当库"两套系统各管一段、还得你自己焊接两边的一致性，是完全不同的活法——它一套系统内部就把"快"和"不丢"一起包圆了。**类比失效边界**：前店后厂省心，代价是"店和厂强绑定、只能一起扩缩容"，不像缓存和 DB 那样能各自独立伸缩；而且门店里的数据在还没搬进仓库的那个窗口里仍有丢失风险，得靠主备副本来兜底。
...
**架构三层**：管理面调度 / 接入代理无状态冗余 / 存储节点主备热备
**六根支柱**：接入无状态 / hash分片打散 / 冷热LRU融合 / 主备热备 / 单key串行乐观锁 / 不停服扩缩容
**命门**：分表因子取高离散度字段（uid），选错（性别/zone_id）分布式白做
**游戏定制**：分服 / 合服 / 回档 / 快照，通用 NoSQL 没有
...
分表因子（Splittable Key）是什么？选错会有什么后果？

<details><summary>参考答案</summary>

分表因子是主键的**子集**，其字段参与 **hash 计算**决定记录落到哪个存储节点。必须取**高离散度**字段（如 uid）。选错（如性别→只分2节点、zone_id→热门区打爆单节点）会把分布式能力锁死甚至撑爆磁盘，**分布式白做**。
...
六根支柱：① **接入代理无状态**可水平扩展 ② 分表因子 **hash 分片**把写压力打散到 N 台（单机10w × N分片）③ **冷热 LRU 融合**用内存扛并发、SSD 扛容量 ④ **主备热备**保99.999%可用 ⑤ 单 key 串行+版本号乐观锁保写一致 ⑥ 不停服扩缩容撑运营洪峰。

</details>

它是怎么防止抽卡/交易类高并发写"超卖"的？为什么不用分布式锁？

<details><summary>参考答案</summary>

靠**记录版本号乐观锁**：单 key 的写在存储节点工作线程内**排队串行**，第一个写成功版本 v10→v11，其余请求带的还是 v10 与服务端不一致→**全部报错**。N 个并发写只有第一个成功。因为单 key 天然串行+CAS 校验，**无需重的分布式锁**。

</details>

对比：为什么不直接用 "Redis + MySQL 拼装"，而要用缓存落地融合的自研存储？

<details><summary>参考答案</summary>

Redis+MySQL 是**两套系统两套一致性**，业务要手写缓存旁路、处理**穿透/击穿/雪崩**，还要运维两份，成本高约40%。融合方案把两层**融进一个进程**：热数据内存、冷数据 SSD 按 LRU 交换，业务只有**一套 API 一套一致性语义**，更省更简单。

</details>

本地索引和全局索引有什么区别？全局索引有哪些限制？

<details><summary>参考答案</summary>

**本地索引**基于主键字段、随表建、支持部分主键查询。**全局索引**是独立系统、**异步准实时**（<1s），支持任意字段范围/模糊/聚合/分页（SQL）。限制：仅 **Generic 表**、单查询最多3000条、limit+offset≤10000、不支持 **join/union/order by/group by/子查询**。
