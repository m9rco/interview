...
> **打个比方**：互联网限流像**高速收费站**——车太多就少开几个道、超载的直接劝返（拒绝 / 429），反正每辆车互不相干，赶回去重来也就重来了。游戏限流像**热门游乐园**——你总不能把兴冲冲进园的玩家"劝返"（体验直接爆炸），只能发号、排队、分批放行（排队机），因为园区里所有人共享同一个世界（同一副本、同一排行榜、同一场活动），必须**控节奏**而不是简单拒绝。**类比失效边界**：这个对比不是说"游戏就不用令牌桶了"——游戏的接入层、跨服网关照样拿令牌桶挡洪水（桶算法详见 [令牌桶与漏桶](/game-infra/token-leaky-bucket.md)）。差别只在**面向玩家的那一层**：互联网对独立请求可以干脆利落地拒，游戏因世界状态耦合，更多是把"拒绝"翻译成"排队 / 帧摊派"这类延迟，让玩家看到的是转圈等待而非一记闭门羹。
...
> 四大算法的**逐行实现、数学推导与 Redis+Lua 落地**详见 [令牌桶与漏桶](./token-leaky-bucket.md)；限流之后的**熔断/降级/兜底**详见 [限流与熔断](./ratelimit-circuitbreak.md)。本篇聚焦"互联网 vs 游戏"两种世界的**选型与形态差异**，不重复展开桶算法细节。
...
漏桶：恒定流出速率，不允许突发，适合严格匀速下游（DB写入）；令牌桶：恒定填充速率，允许桶内令牌积累消费突发，适合大多数 API 限流场景。关键差异：令牌桶**允许突发**，漏桶**不允许**。

</details>

2. Redis + Lua 分布式令牌桶，为什么用 Lua 脚本而不是普通命令序列？
<details><summary>参考答案</summary>
...
3. 游戏服务器为什么不用 429，而是用排队和 CD 来表现限流？
<details><summary>参考答案</summary>
...
4. 活动开服时「排队机 + 平滑扩容」方案，为什么扩容时不能全量放行？
<details><summary>参考答案</summary>
...
5. 对比「中心配额分发」和「Redis+Lua 单 key 限流」两种方案，各有什么取舍？
<details><summary>参考答案</summary>
