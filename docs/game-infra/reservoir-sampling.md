...
> **打个比方**：把蓄水池想成一间只有 `k` 个座位的房间。第 `i` 个人进门时，抛个骰子——以 `k/i` 的概率决定"要不要挤掉现有某位、自己坐下"。凭啥这么做最后人人机会均等？因为越晚进门的人，坐下的概率（`k/i`）越低；可越早坐下的人，被后来者挤走的机会也越多。一个"进得难"、一个"待不稳"，两股力恰好抵消，最终**每个人留在座位上的概率都精确等于 `k/n`**——哪怕你自始至终都不知道总共会来多少人。**类比失效边界**：这份漂亮的公平是**等概率、无权重**的。一旦每个元素该被抽中的机会不一样（比如 VIP 玩家的日志更该留样），朴素蓄水池就失灵了，得换成加权变体（A-Res / A-ExpJ）——那时"抛骰子"的概率不再是简单的 `k/i`，而要按权重重新推导。
