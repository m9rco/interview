...
> **打个比方**：接入网关就像大公司的**总机前台**。所有外线电话（玩家长连接）统统打到前台，由前台统一接听、登记、验明身份，再按分机号转接给对应的内部部门（GameSvr / 场景服 / 匹配服）。这样各部门只管埋头干活，不用每人自己拉一根外线、装一部电话（业务进程无需直接 `accept()` 海量连接）；前台还顺手帮你挡骚扰电话（鉴权 / 限流）、你断线了也帮你记着聊到哪儿、方便重连。**类比失效边界**：前台是所有外线的**唯一必经之路**——它一趴窝，整个公司对外瞬间失联（网关是有状态的单点风险）。所以网关绝不能只有一台，必须**集群化**，且连接状态要能迁移或快速重建，否则"帮业务卸包袱"就退化成"给系统埋新单点"。
