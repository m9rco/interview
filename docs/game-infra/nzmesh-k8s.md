...
- **两级灾备**：`STBusRoute` 有 `backup_route_id`（route 全挂切别的 route）和 `backup_zone_id`（zone 全异常切别的 zone），回包失败时改写 header 重路由
...
- **集群清单**（`schedulersvrd/k8s/k8s_cluster/`）：`nju.yaml`=南京、`sh.yaml`=上海，各一套 apiserver 证书
...
- **K8s API**（client-go）：为每个集群建 clientset，`Nodes().List()` 拉节点、`Pods().List()` 拼 tbus 地址
- **对账**（`get_nodes.go:311`）：在 host.txt 却未组网 / 已组网却不在 host.txt / DS 节点未跑 dsagent，分别告警

**生产保护**：所有写操作受 `enable_k8s_op` 总闸控制，dev/test=1，**tiyan/live=0（线上禁止 panel 直接操作 Pod）**。
