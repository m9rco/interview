...
> **打个比方**:checkpoint + WAL 就像**RPG 的存档 + 操作日志**。玩家每隔 5 分钟自动**存档到硬盘**(snapshot,恢复的基线);两次存档之间的每一次击杀、拾取、移动写进"操作日志"(WAL);宕机后先读**最近那次存档**,再把日志里的操作按顺序**回放到当时那一刻**,玩家几乎无感。**类比失效边界**:这一切只对**确定性回放**成立。如果日志里有"当时随机掉了个史诗装备"、"当时读了另一个玩家的实时位置"这类**依赖外部状态的副作用**,重放会算出**不一样的结果**——同一条"击杀 boss"操作,第一次掉紫装、恢复重放时掉蓝装,玩家就要炸了。工程上的解法只有两条:要么把**随机种子、外部依赖读值**一起写进 WAL(让重放严格复现),要么把"不可重放"的副作用挪到 checkpoint 之外做**补偿**——绝不能指望"只记操作、不记环境"就能一键回滚宇宙。
