...
> **打个比方（有栈 vs 无栈）**：**有栈协程**像给每个协程配一整间**独立办公室**（一条完整的栈）——你随时能起身离开去干别的，桌上摊开的文件原封不动，回来接着干（任意深处都能挂起、老代码一行不改）。缺点是办公室太占地，几十万个协程就是几十万间房，内存吃不消。**无栈协程**则不给固定办公室，只发你一个"**折叠工位包**"：要挂起时，把桌面上**必需的那几样**塞进一个定长小包带走（现场被编译成定长帧），省地方、百万并发也塞得下。但代价是——你的活必须**提前登记成"可打包"的**，也就是每个挂起点都得显式写 `co_await`。**类比失效边界**：这一"登记"会像感冒一样传染——某个函数染上了"异步"这个颜色（变成 `co_await` 的），调用它的函数也被迫跟着染成同色，一路蔓延整条调用链，这就是**函数染色**。反过来有栈协程没有染色烦恼（零改造），却要为每个协程预留栈、吃内存还可能栈溢出。所以这是"省内存但染色（无栈）vs 省心但费内存（有栈）"的对立，天下没有免费的协程。
