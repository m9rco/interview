...
> **打个比方（DNS 隧道，防御视角）**：DNS 查询本该只是查电话簿——"问一下 `example.com` 的 IP 是多少"。而 DNS 隧道好比有人把这套"查号"流程当成夹带纸条的暗道：把要外带的数据编码进域名的**子域部分**（于是出现大量看似乱码的超长子域查询），而那台"权威服务器"其实是攻击者自己控制的，会在"回答"里再夹带指令回来。正因为 53 端口几乎全网放行、又没人逐条细看查询内容，这条暗道才格外隐蔽。**类比失效边界**：所以检测**不能靠看"内容"**——编码后本就是一串乱码域名，你读不出意义；只能靠**行为统计特征**把它揪出来：某主机 DNS 查询量骤增、子域名的字符熵异常高、TXT/NULL 记录多得离谱、单一父域下涌出海量随机子域。防御的落点则是"出网收敛"——强制内网只能走指定递归解析器，把这扇门尽量关小。
...
UDP DNS 只靠什么来绑定"请求-响应"？为什么这成了缓存投毒的攻击面？

<details><summary>参考答案</summary>

只靠 **16-bit 事务ID + 源端口 + 目的端口 + Question** 四要素的熵。无连接无握手，若**源端口固定**则可猜测熵仅 16 bit（6.5万种），攻击者可抢在真实权威前伪造响应抢答。防御是**端口随机化 + 0x20 大小写混淆**提熵，**DNSSEC** 用签名根治。

</details>

Kaminsky 攻击相比传统缓存投毒的"放大点"在哪里？

<details><summary>参考答案</summary>

传统投毒一次 miss 只有一个抢答窗口，猜错要等 TTL 过期。Kaminsky 不停查询 **`随机N.bank.com`**，每次必然 miss 触发向权威的查询，从而获得**海量抢答窗口**；一旦某次伪造响应用 **Additional 区段**塞进 NS/glue，就能污染整个域而非单条记录。

</details>

DNS 隧道能穿透多数企业防火墙的物理基础是什么？

<details><summary>参考答案</summary>

递归解析的**"任意子域都会被如实转发到对应权威"**特性。企业出口通常**放行53端口且不深检 DNS 内容**；即便封直连，内网主机→内网递归器→外部权威这条**合法链路**依然通畅，数据被编码进子域"驮"出去。

</details>

对比：检测 DNS 隧道用"静态签名"还是"行为统计"更好？为什么？

<details><summary>参考答案</summary>

**行为统计更鲁棒**。隧道的编码字母表、分片大小可任意调整，静态签名/域名黑名单易被绕过且永远滞后。而**高熵子域 + 低缓存命中 + 异常QTYPE分布 + 长会话**是隧道为传输效率与穿透性付出的代价，难以同时消除，因而更难规避。
...
53 端口**业务必需**，一封全断；且隧道可走 **DoH/DoT（443/853）**伪装成 HTTPS 绕过纯端口策略。正确做法是**收敛出网路径**（只允许经受控递归器出网、封主机直连），再在递归器/出口做**内容与行为层面**的检测。
