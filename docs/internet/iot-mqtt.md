...
> **打个比方（MQTT 发布订阅 + QoS）**：MQTT 像订报纸。设备（发布者）把消息按"主题"（topic，像报纸名）投给 Broker（邮局），谁想收哪类就去邮局**订阅**，邮局负责分发——发的人不用知道谁在收、收的人也不用认识谁在发（彻底解耦）。而三档 **QoS** 就是三种寄信方式：**QoS0** 是平邮（寄了就不管，可能丢，最省）；**QoS1** 是挂号信要回执（保证至少送到一次，但可能重复送）；**QoS2** 是最严格的双向确认（恰好一次，但要来回四次握手、最慢最贵）。**类比失效边界**：QoS 越高越可靠，但也越费流量和电——对每字节计费、电池当命的 IoT 设备，无脑全上 QoS2 等于自杀。所以大多数周期性遥测数据用 QoS0/1 足矣，只有"开锁""关阀"这类绝不能丢也绝不能重的关键指令才值得用高 QoS。
