...
> **打个比方**：Nginx 是餐厅的**前台领位员**——一个人就能笑脸招呼大堂里上万个排队、闲聊、发呆的客人（epoll 扛海量空闲连接），因为他只管"招呼、引导"，从不亲自下厨。php-fpm 则是**后厨的几个厨师**——每人一次只能专心炒一道菜（阻塞式 worker），炒完这道才接下一道，靠增加人手来提高出菜量。前台**绝不能亲自去炒菜**，否则他一钻进后厨，整个大堂瞬间没人招呼、全堵死（event loop 被 CPU 密集任务卡住）——这正是两种相反并发模型必须配对的原因。**类比失效边界**：后厨厨师数量（fpm 的 `max_children`）是**硬上限**。客人再多、前台再会招呼，后厨就那么几个人——点菜单堆过某个量就只能干等。所以 worker 数得按 CPU 核数和单进程内存精算：配太多会把内存吃爆、拖垮整机，配太少则请求排队直到超时。
