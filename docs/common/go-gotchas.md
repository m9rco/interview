...
> **打个比方（slice 共享底层数组）**：slice 不是数组本身，而是一张"租约表头"——记着"从哪栋公寓楼的哪个房间起租、租了几间（len）、这层楼总共有几间（cap）"。把 slice 传给别人是复制这张表头，可**两张表头租的还是同一栋楼**。于是你在自己租的房间里砸了堵墙，隔壁租了重叠区域的人也跟着遭殃（改一处、动全局）。`append` 时若楼里还有空房（未超 cap），就直接续租、大家继续共用一栋楼；一旦住满触发扩容，Go 会**盖一栋新楼把你整个搬过去**，从此你和老邻居再无瓜葛（分离）。**类比失效边界**：到底"共享"还是"分离"，全看这次 `append` 有没有触发扩容，而扩容时机又取决于 cap——所以同一行 `append` 代码，有时动原数组、有时不动，行为很坑。要想稳，就显式 `copy` 一份，或用三索引切片 `s[a:b:b]` 把 cap 焊死。
