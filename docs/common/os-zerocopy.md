...
> **打个比方（零拷贝）**：传统的"读文件再发网络"（`read`+`write`）像一次蠢搬家——把一箱货从仓库（磁盘）先搬进你家客厅（内核 Page Cache），再吭哧吭哧扛到卧室（用户态缓冲），可你**看都没看一眼**，又原样扛回客厅（socket 缓冲），最后才装车发走（网卡）。这箱货压根没在你卧室干任何事，却白白多搬了两趟、还惊动你两次（4 次拷贝 + 4 次上下文切换）。`sendfile`/`splice` 就是让转运中心**直接从仓库发车**，绕过"扛进卧室又扛出来"这段冤枉路，且全程由 DMA 这个"搬运工"代劳、CPU 连手都不用伸。**类比失效边界**：零拷贝省的是"你根本不用经手的数据"，前提是你**确实不需要在用户态加工它**（静态文件、视频下发最典型）。一旦你要在用户态动这批数据（加密、压缩、改模板），它就非进你卧室不可——零拷贝当场失效，老老实实拷贝。
