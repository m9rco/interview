...
> 打个比方看懂三者的区别：**进程**是各自独立的公司——有自己的办公楼和资源，互不干扰，但你想让两家公司协作（跨进程通信）就得打电话发传真，麻烦又慢。**线程**是同一家公司里的员工——共用一间办公室和文件柜，沟通快，但两个人同时去抢同一份文件就会打起来（数据竞争、死锁）。**协程**则是一个员工自己手上开着好几个任务，干这个干到要等外卖（I/O 阻塞）时，自己主动切去干那个，全程不用惊动经理（内核）——所以又轻又快。**类比失效边界**：员工自己切任务的前提是他"愿意主动让出"；一旦某个任务一头扎进 CPU 死算不歇（CPU 密集）或调了个阻塞式系统调用不返回，它就霸着位置不切给别人——这正是协程对 CPU 密集/阻塞调用的软肋，得靠多线程/多进程或把调用异步化来补。
