...
**打个比方（EWMA）**：EWMA 像人的记忆——越近发生的事印象越深、越久远的越淡（权重指数衰减）。所以它描出的"正常值"会**随业务节律自动漂移**：白天高峰它跟着抬高、半夜低峰它跟着压低，报警线 `μ±kσ` 是浮动的，绝不会像一条固定阈值那样"白天狂误报、半夜装睡漏报"。**类比失效边界**：这种"记忆型"基线擅长跟随**平缓变化**，但对真正的**阶跃突变反应迟钝**——它会先把突刺当噪声压一压，等回过神来黄花菜都凉了。所以秒级尖刺不能指望 EWMA，得另配一个"短窗 vs 长期基线"双尺度对比的哨兵；而"磁盘几小时后写满"这种要抢提前量的活，又得交给会做趋势外推的 ARIMA。三者各治一段时间尺度，这才是要分层的根本原因。
