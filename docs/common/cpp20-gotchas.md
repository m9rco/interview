...
> **打个比方（Ranges view）**：view 是一张"菜谱 / 待办清单"，不是一盘已经做好的菜。你写 `vec | filter | transform`，只是列了个"先筛后炒"的计划，此刻一口都还没做——直到你真正去遍历它（开火），才现点现炒、逐个元素求值。正因如此，view **不拥有食材**：底层那个 `vec` 要是先被扔了（销毁），你手里的菜谱再漂亮也炒不出菜（悬垂）。**类比失效边界**：有些菜谱是"一次性"的（有状态 view，如 `filter`），因为每次遍历都重新开火现算，遍历两遍可能得到不一样的结果、甚至只能过一遍——所以别把 view 当成能反复端上桌、随便存起来跨函数传的成品。
