...
> **用烧水一眼记住四象限**：**同步阻塞** = 你死死站在灶台前盯着水，水不开你啥也不干；**同步非阻塞** = 你去客厅玩手机，但得每隔一会儿自己跑回厨房瞄一眼开没开（轮询，跑腿累死）；**异步阻塞（多路复用）** = 你同时烧了好几壶水，搬把椅子守在厨房，哪壶先响你就去处理哪壶（`epoll` 帮你一次盯多壶，但你人还是得守着）；**异步非阻塞** = 你用了个会自己吹哨的水壶，烧上就回房睡觉，水开了它自己叫你（内核干完活主动通知，`io_uring` 才是这种）。**类比失效边界**：别被"异步阻塞"这个叫法带偏——`epoll` 的"异步"只是"一次能盯多壶"，那个"读水"的动作最后仍是你自己同步去舀的；真正把"舀水"这活儿甩给内核异步完成的，只有 `io_uring`/AIO。
