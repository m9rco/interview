...
**打个比方**：暴力匹配像个没记性的侦探——跟踪嫌疑人走了 5 步发现认错人，就懊恼地跑回起点从头再跟。KMP 是有记性的侦探：他记得"刚才对上的这 5 个字里，开头一小段和结尾一小段长得一模一样"，所以认错人时**不退回起点**，直接把这段已知信息平移接上继续跟——侦探的脚（文本指针）永远只向前迈。`next` 数组就是他提前做好的功课：模式串每个前缀"最长的相同前后缀有多长"。**类比失效边界**：这份省力全靠**模式串 P 自身的结构**、与文本 T 无关，所以 `next` 只在 P 上预处理一次、换文本不必重算——但一旦换了要找的模式串，功课就得从头重做。
