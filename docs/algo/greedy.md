...
**打个比方**：贪心就像一个只看眼前、绝不回头的吃货——每一步都抓当下最大的那块蛋糕。有时候这么干真能吃到最多（活动安排、Huffman 编码），有时候却因小失大：经典反例是找零，用 `{1,3,4}` 元的硬币凑 6 元，贪心先抓最大的 4、剩 2 只能 `1+1`，总共 3 枚；而稍微动脑就看出 `3+3` 只要 2 枚。**类比失效边界**：贪心对不对，**绝不能靠"感觉很合理"拍脑袋**，必须用交换论证严格证明"局部最优能拼出全局最优"。证不出来（像上面的找零币制）就别硬贪，老老实实上 [动态规划](./dynamic-programming.md)。
