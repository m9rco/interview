...
**打个比方（扫描线）**：它就像停车场的进出闸机。每辆车开进来计数器 +1、开出去 -1，你压根不用管具体是哪辆车——只要沿时间轴扫一遍，计数器的**峰值**就是"最多同时停了几辆"，也就是至少要备几个车位（= 至少几间会议室）。区间 `[s,e]` 拆成 `s` 处 +1、`e` 处 -1 两个事件，正是这个闸机计数。**类比失效边界**：闸机只数得出"此刻有几辆"，答不了"到底是哪几辆车挤在一起"。要还原具体是哪些区间相交，就得回头保留每个区间的身份另作处理——光盯着差分数组那串数字是问不出来的。
