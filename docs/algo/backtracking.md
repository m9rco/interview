...
**打个比方**：回溯就像走迷宫时手里牵一根线、沿途在地上做记号。走进死胡同了，就顺着线退回上一个岔路口，**并把刚才走这条路做的记号擦掉**，再换另一条岔路试。"做选择 → 递归 → 撤销选择"这套三连里，最容易被新手漏掉的正是最后那步"擦记号"。**类比失效边界**：迷宫里的路是物理存在的，你退回去它自然还在；但代码里的"撤销"（restore）必须**你亲手显式写出来**——一旦忘了撤销，上层就会带着下层残留的脏状态（比如 `path` 里没弹出的元素、`used` 没复位的标记）去试别的分支，结果满盘皆错，还特别难 debug。
