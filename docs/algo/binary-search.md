...
**打个比方**：很多人以为二分的前提是"数组必须排好序"，其实真正的前提是**存在一条单调的分界线**——一边全是"不行"、一边全是"行"。就像洗澡拧水龙头调水温：你不知道确切的角度刻度，但你知道"太凉就往热拧、太烫就往凉拧"，每次朝舒服的方向砍掉一半范围，几下就逼到临界点。"数组有序"只是这种单调性最常见的一种长相罢了。**类比失效边界**：水温是连续的、可以无限逼近，而二分面对的是**离散整数区间**——逼到 `lo > hi` 必须收手，且"相等时往哪边收"直接决定你找到的是左边界还是右边界，手一抖就是经典的 off-by-one。
