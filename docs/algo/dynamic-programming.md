...
**打个比方**：不用 DP 的暴力递归有多蠢？就像你每次想吃西红柿炒蛋，都要从播种西红柿开始重新种一遍——算 `fib(5)` 要算 `fib(4)+fib(3)`，算 `fib(4)` 又要算 `fib(3)+fib(2)`……同一个 `fib(3)` 被从头重算无数遍。**记忆化**就是"算过的记在本子上"：每级台阶贴张便利贴写下答案，后面路过直接抄；**自底向上的递推**更绝——干脆按顺序把便利贴一张张填满，连递归调用的开销都省了。**类比失效边界**：便利贴能直接抄的前提是"子问题的答案不随谁来问、从哪条路来而改变"（无后效性）。要是 `dp[i]` 的最优解会因为你走的是哪条路径而不同，这张便利贴就不能简单复用——得把"状态"定义得更细（多加一维）才救得回来。
