...
> 想象你在饭店当传菜员：10 张桌子编号 0~9，客人的菜按"订单号 % 10"上桌，岁月静好。突然老板加开第 11 桌——灾难来了，几乎每桌的菜都要重算落到哪，全场跑堂原地爆炸。这就是**取模分片**扩容时的死法：`hash % N` 里 N 一变，几乎所有 key 换位置，缓存瞬间全失效、DB 被穿成筛子。下面这堆算法，一半是来治这个"多加一张桌就全乱"的病的。
...
**打个比方**：把哈希环想成一个圆形钟表盘，每个节点是钉在盘面上的钉子，key 是掉进盘里顺时针滚的小球——球撞到的第一个钉子就归谁管。新钉一颗钉子进去，只有"从它逆时针到前一颗钉子"这段弧上的球改换门庭，别处的球压根没察觉老板换了人。**类比失效边界**：真实的环是 2³² 的哈希空间而非 12 小时刻度，而且每颗钉子还要"分身"成上百个虚拟节点撒满整圈，否则三五颗钉子会让球分布严重偏心（这也是虚拟节点存在的理由）。
