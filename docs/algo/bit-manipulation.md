...
**打个比方（异或消偶）**：`^` 就像一场"配对消消乐"。一堆数里，每个都恰好出现两次、只有一个落单——把它们全异或起来，成对的两两相遇当场抵消归零（`a^a=0`），最后亮在屏幕上的就是那个"单身狗"。因为异或满足交换律、结合律，谁先谁后消都无所谓，一趟扫过去 O(1) 空间搞定。**类比失效边界**：这招干净利落的前提是"其余元素**恰好成对（偶数次）**、仅一个落单"。要是有两个落单、或某个数出现三次，单纯全体异或就失灵了——得改用"分组异或"或"按位统计出现次数再 mod 3"这类更细的办法。
