...
**打个比方**：BFS 像往池塘正中丢一块石头，涟漪一圈圈同心扩散——每扩一圈距离就 +1，所以在不带权的图里，涟漪**第一次碰到目标的那一刻就是最短路**。Dijkstra 则像给粗细不同的水管网注水：水总是先灌满"当前离源头最近、还没封口"的那个节点（用小根堆每次挑出它），封一个、松弛一圈邻居，再挑下一个。**类比失效边界**：Dijkstra"先灌满的就封口、永不反悔"这套，**死死依赖边权非负**。一旦出现负权边，后面可能冒出一条"绕远却总权更小"的路，把之前拍板封口的最短距离推翻——这时候 Dijkstra 就会给出错误答案，得改用 Bellman-Ford / SPFA。
...
1. 给定一张不带权图求单源最短路，为什么用 BFS 而不是 DFS？BFS 靠什么保证"首次访问即最短"？
<details><summary>参考答案</summary>

队列 **FIFO** 保证**按层扩散**，第 k 层节点到源距离恰为 k，故**首次出队即最短**。DFS 会先走深再回头，先到的不一定最近。

</details>

2. Dijkstra 和 Bellman-Ford 都求单源最短路，二者如何取舍？
<details><summary>参考答案</summary>

**Dijkstra** 贪心 + 小顶堆，**O(E log V)**，但要求**非负权**（负权破坏"最近点已定"前提）。**Bellman-Ford** 松弛 V-1 轮，**O(VE)**，可处理**负权并判负环**，但更慢。

</details>

3. 判断有向图是否有环有哪两种做法？拓扑排序如何同时给出顺序？
<details><summary>参考答案</summary>

①**拓扑排序**（Kahn 入度法）：入度 0 出队、删边，若输出节点数 **<n** 则有环，出队顺序即拓扑序；②**DFS 三色**标记，遇**灰色回边**即有环。

</details>

4. 并查集为什么单次操作近 O(1)？两大优化各起什么作用？
<details><summary>参考答案</summary>
...
5. 存图选邻接表还是邻接矩阵？各自适合什么场景？
<details><summary>参考答案</summary>

**邻接表**省空间、遍历邻居快，适合**稀疏图**；**邻接矩阵** O(1) 查任意边、实现简单，适合**稠密图**或频繁查边。
