...
> 排序是最容易被"背了个快排就以为会了"坑到的题。面试官真正想看的是：你会不会**先问清楚场景再动手**。同样是排序，给你 100 个近乎有序的数、给你 10 亿条带重复的日志、给你一条不能乱序的链表——最优解完全不同。上来就手写快排的人，和先反问四个问题的人，段位一眼可辨。
...
- **没有银弹**：比较排序下界 O(n log n)，"最快"取决于输入。工程库用混合策略在各种输入上都不塌方（C++ `std::sort` = introsort：快排 + 递归过深转堆排兜底 + 小区间插入收尾）。
  - **为什么下界甩不掉（打个比方）**：把排序想成玩"20 个问题"猜谜。n 个元素一共有 `n!` 种可能的排列，而每做一次比较，本质是问一个是/否问题、最多把候选排列砍掉一半。要从 `n!` 种里锁定唯一正确的那一种，至少得问 `log₂(n!) ≈ n log n` 个问题——答案空间越大，逃不掉的提问次数越多。**类比失效边界**：计数/基数排序快过这个下界，不是因为它更聪明，而是它**根本不玩"比较"这个游戏**（直接按值分桶），所以不受"20 个问题"规则约束——代价是只适用于 key 范围可控的整数/定长键。
